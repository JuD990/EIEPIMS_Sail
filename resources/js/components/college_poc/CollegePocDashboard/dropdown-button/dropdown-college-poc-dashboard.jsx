...
const DashboardDropdown = ({ setSelectedDepartment, setSelectedSchoolYear, setSelectedSemester }) => {
    ...
    const fetchDepartments = async () => {
        try {
            const response = await apiService.get("http://localhost:8000/api/getDepartmentsOptions");
            ...
            if (employeeId && userType) {
                try {
                    const userDeptResponse = await apiService.get(
                        `/employee-department/${userType}/${employeeId}`
                    );
